...
// Scanner represents a lexical scanner for kQL.
type Scanner struct {
	r *reader
}
...
func (s *Scanner) Scan() (tok Token, pos Pos, lit string) {
	...
	if isWhitespace(ch0) {
		...
		if ch0 == '%' {
			ch1, _ := s.r.read()
			if isLetter(ch1) {
				// Read an identifier prefixed with '%'
				s.r.unread()
				s.r.unread()
				return s.scanIdent()
			}
	...
	case '~':
		if ch1, _ := s.r.read(); ch1 == '~' {
			if ch2, _ := s.r.read(); ch2 == '*' {
				return EQLIKECASE, pos, ""
			}
			s.r.unread()
			return EQLIKE, pos, ""
		} else if ch1 == '*' {
			return EQREGEXCASE, pos, ""
		}
		s.r.unread()
		return EQREGEX, pos, ""
	case '!':
		if ch1, _ := s.r.read(); ch1 == '=' {
			...
			if ch1, _ := s.r.read(); ch1 == '~' {
				if ch2, _ := s.r.read(); ch2 == '*' {
					return NEQLIKECASE, pos, ""
				}
				s.r.unread()
				return NEQLIKE, pos, ""
			} else if ch1 == '*' {
				return NEQREGEXCASE, pos, ""
			}
			s.r.unread()
			return NEQREGEX, pos, ""
		}
...
func IsRegexOp(t Token) bool {
	return (t == EQREGEX || t == NEQREGEX || t == EQREGEXCASE || t == NEQREGEXCASE || t == EQLIKE || t == NEQLIKE || t == EQLIKECASE || t == NEQLIKECASE)
}

// IsLikeOp returns true if the operator accepts a LIKE operand.
func IsLikeOp(t Token) bool {
	return (t == EQLIKE || t == NEQLIKE || t == EQLIKECASE || t == NEQLIKECASE)
}
